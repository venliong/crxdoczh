...
class ServerInstance(object):
  ...
  def Get(self, path, request, response):
    ...
    import logging
    logging.info('Server Instance')
    logging.info(path)
    if path == 'api/samples/apps':
      response.headers['content-type'] = 'text/plain'
      response.set_status(200)
      response.out.write(templates._samples_data_source.get_json('apps'))
      return
    elif path == 'api/samples/extensions':
      response.headers['content-type'] = 'text/plain'
      response.set_status(200)
      response.out.write(templates._samples_data_source.get_json('extensions'))
      return
